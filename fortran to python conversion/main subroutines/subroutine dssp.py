...
def dssp(c, n1, n, nslt, line, index, inamcol1, inamcol2, iresncol1, iresncol2, nneig, ineig, nbox, indices, ihbneig, ixc, ixo, ixn, ixa, dssplab, idistdssp, ch, cres, enghb, iparal, iantiparal, nss, itypss, ifss, ilss, ires0, nconfig, iwdssp, iwrose, iwhead, ifail, radtodeg, maxrepconf, maxng, nnlistlen, maxbox, listlen, maxss, maxrsd, maxrec):
    ...
    ***
    ...
    # Divides atoms between:
    # Simple carbon atoms (ixc)
    # Oxygene atoms (ixo)
    # Nitrogen atoms (ixn)
    # Alpha carbon atoms (ixa)
    # Hydrogen atoms (ch) which deserve a special treatment
    # Solvent atoms (suppress from the s)
            ...
            if (ireso != 0):
                if (icfound*infound*iofound < 1):
                    ...
                    if (iccprevf*iafound == 1):
                        ...
                        dnc = 0.0
                        for k in range(3):
                            rx[k] = 2.0*c[k][ixn[nres]] - \
                                c[k][ixa[nres]]-ccprev[k]
                            dnh = dnh+rx[k]**2
    ...
    if (nresok == 0):
        ...
        return res
    #
    # All the lines above seem only to parse the coordinates
    # That should be done in a seperate function
    # We can also note that these lines manage exceptions where there
    # would not be C, N and CA
    #
    #  Calcutating Hydrogene bonds energies
    threshold = -0.5/(0.42*0.20*332.0)
    for ir in range(nres):
        enghb[ir] = threshold+10**(-5)
        dssplab[ir] = ' '
        # if there is at least a carbon
        if (ixc[ir] > 0):
            cres[0][ir] = c[0][ixc[ir]].copy()
        else:
            cres[0][ir] = np.zeros(3)
    rchb = 9.2
    nnlistsim(1, nres, cres, nneig, ineig, indices, nbox, rchb,
              ifail, maxng, nnlistlen, maxbox, listlen, 0)
    if (ifail > 0):
                ...
                for jr in range(ir+3, nres):
                    if (ihbneig(jr) >= 0):
                        if (dist2(c[0][ixc[ir]], c[0][ixc[ir]]) < rchb**2):
                            eij1 = 1.0 / \
                                math.sqrt(dist2(c[0][ixc[ir]], c[0][ixc[ir]]))
                            eij2 = 1.0 / \
                                math.sqrt(dist2(c[0][ixc[ir]], ch[0][jr]))
                            eij3 = -1.0 / \
                                math.sqrt(dist2(c[0][ixc[ir]], ch[0][jr]))
                            eij4 = -1.0 / \
                                math.sqrt(dist2(c[0][ixc[ir]], c[0][ixc[ir]]))
                            ...
                            eji1 = 1.0 / \
                                math.sqrt(dist2(c[0][ixc[ir]], c[0][ixn[ir]]))
                            eji2 = 1.0 / \
                                math.sqrt(dist2(c[0][ixc[ir]], ch[0][jr]))
                            eji3 = -1.0 / \
                                math.sqrt(dist2(c[0][ixc[ir]], ch[0][jr]))
                            ...
                            if (eji < enghb[jr]):
                                ...
                                ihbneig[jr] = ir
                                enghb[jr] = eji
    # Same code as the "if" that started before
    # Should be transformed into a function
    else:
        for ir in range(nres):
            if (ihbneig[ir] >= 0):
