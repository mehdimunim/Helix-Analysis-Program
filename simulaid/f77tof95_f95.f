c*****Preprocessor to generate the source code of the program MMC
      character(len=80) line,line1,blankline,filename
      character(len=2) sym
      character(len=4) SYSTEM
      character(len=5) tfl(2)
...
1001  format(a1)
      ...
      character(len=*) q  
      character(len=132) ansline,pline
      lenq=len
      pline(1:1)=' '
      pline(2:lenq+1)=q(1:lenq)
...
c     Finds the next blank in line
      ...
      block data
      common /values/ isize(70),iopt(30),maxo,maxs
      character(len=2) sizesym,optname
      character(len=10) sizename
      character(len=25) optlname
      character(len=38) sizelname
