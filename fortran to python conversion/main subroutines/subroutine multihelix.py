  ...
  for ihx in range(nhxres):
    ...
    for jhx in range(ihx+1, nhx):
      ...
      if (a1  <  -halflen1  or  a1  >  halflen1  or a2  <  -halflen2  or  a2  >  halflen2) :
        ...
        if (a1  <  -halflen1) :
          # lines in comment in multhelix.f
        if (a2 < -halflen2):
          # lines in comment in multihelix.f
        if (d2min < 99999.0):
          print(" NOTE: closest HX {} - HX {} contact is outside",ihx,jhx)
        else:
          if (noproj == 0):
            print(" NOTE: mismatch between  HX {} - HX {} ",ihx,jhx)
          if (noproj == 1):
            print("NOTE: no overlap between HX {} - HX {} ", ihx, jhx)
        angrot1=0
        if (icbahx > 0):
          torsats[0][3] = c[0][icbahx[ihx]]
        ...
        angrot2 = 0
        if (icbahx[jhx] > 0):
          c[0][icbahx[jx]] = torsats[0][3]
          torsats[0][2], a = project(c(1,icbahx(jhx)),cent2,ax2)    
          ...
          torsats[0][1] = ct1
          torsats[0][2] = ct2
          torsats[0][0] = paramx(ct1, ax1, halflen1)
          torsats[0][3] = paramx(ct2, ax2, halflen2)
          ...
          torsats[0][1] = cent1
          torsats[0][2] = cent2
          torsats[0][0] = paramx(cent1,ax1,halflen1)
          torsats[0][3] = paramx(cent2,ax2,halflen2)
          ...
          print(" HXs# {} - {} dist= {} cc_dist = {} dee1 = {} dee2 = {} {}".format(ihx,jhx,dist,dist_cc,distss,disthh,ap_pa(isg2ij+2)))
          print(" HXs# {} - {} ang= {} ang= {} dhang= {} dhang_cc = {} CAs: {} {}".format(ihx,jhx,ang,dhang,dhang_cc,iamin1,iamin2))
          print(" HX {} rotation wrt HX {} = HX {} rotation wrt HX {} = {}".format(ihx,jhx,angrot1,jhx,ihx,angrot2))
          return